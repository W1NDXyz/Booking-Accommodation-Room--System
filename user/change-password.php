...
if(isset($_POST['submit']))
{
	...
	$sql_change_password = "SELECT * FROM user WHERE user_password = '$password' AND user_id = '".$_SESSION['user_id']."'";
	if($result_change_password = $connect->query($sql_change_password))
	{
		if($total_change_password = $result_change_password->num_rows)
		{
			if($confirm_password==$new_password)
			{
				...
				$sql_change_password = "UPDATE user SET user_password = '$new_password' WHERE user_id = '".$_SESSION['user_id']."'";
				if($result_change_password = $connect->query($sql_change_password))
				{
					$_SESSION['message'] = 'change password success';
					header('Location:'.$link.'/user/change-password.php');
					exit();
				}
				else
				{
