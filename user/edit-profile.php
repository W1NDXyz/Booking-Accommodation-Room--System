...
{
	$sql_edit_profile = "SELECT * FROM user WHERE user_email = '$email' AND user_id != '".$_SESSION['user_id']."'";
	if($result_edit_profile = $connect->query($sql_edit_profile))
	{
		...
		{
			$sql_edit_profile = "UPDATE user SET user_name = '$name', user_email = '$email', user_phone = '$phone', user_address = '$address' WHERE user_id = '".$_SESSION['user_id']."'";
			if($result_edit_profile = $connect->query($sql_edit_profile))
			{
				$_SESSION['message'] = 'edit profile success';
				header('Location:'.$link.'/user/edit-profile.php');
				exit();
			}
			else
			{
	...
	<div class="page-wrapper">
		...
		<div class="page-content">
			<nav class="page-breadcrumb">
				<ol class="breadcrumb">
					<li class="breadcrumb-item"><a href="<?php echo $link.'/user/index.php';?>">Dashboard</a></li>
						...
						<div class="card-body">
							...
							<form method="post" class="forms-sample">
								<div class="row mb-3">
									...
									<div class="col-sm-10">
										<input type="text" name="name" class="form-control" value="<?php echo $rows_login['user_name'];?>" required>
									</div>
								</div>
								<div class="row mb-3">
									<label class="col-sm-2 col-form-label fw-bold text-md-end">Email Address <span class="text-danger">*</span></label>
									<div class="col-sm-10">
										<input type="email" name="email" class="form-control" value="<?php echo $rows_login['user_email'];?>" required>
									</div>
								</div>
								<div class="row mb-3">
									<label class="col-sm-2 col-form-label fw-bold text-md-end">Phone Number <span class="text-danger">*</span></label>
									<div class="col-sm-10">
										<input type="text" name="phone" class="form-control" value="<?php echo $rows_login['user_phone'];?>" required>
									</div>
								</div>
								<div class="row mb-3">
									<label class="col-sm-2 col-form-label fw-bold text-md-end">Address <span class="text-danger">*</span></label>
									<div class="col-sm-10">
										<textarea name="address" class="form-control" rows="5" required><?php echo $rows_login['user_address'];?></textarea>
									</div>
								</div>
								<div class="row mb-0">
