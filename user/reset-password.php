...
{
	$sql_reset_password = "SELECT * FROM user WHERE user_email = '$email'";
	if($result_reset_password = $connect->query($sql_reset_password))
	{
		...
		if($total_reset_password = $result_reset_password->num_rows)
		{
			...
			try {
				...
				$mail->addAddress($rows_reset_password['user_email'], $rows_reset_password['user_name']);
				...
				<body>
				<p>Hello '.strtoupper($rows_reset_password['user_name']).',</p>
				<p>There has been a request to reset your password for '.$title.'. The changes as below:</p>
				<p><b>Username</b>: '.$rows_reset_password['user_username'].'<br><b>Password</b>: '.$password.'</p>
				<p>Please access the link below for further action:<br><a href="'.$link.'/user/login.php">'.$link.'/user/login.php</a></p>
				...
				$sql_reset_password = "UPDATE user SET user_password = '$password' WHERE user_id = '".$rows_reset_password['user_id']."'";
				if($result_reset_password = $connect->query($sql_reset_password))
				{
					$_SESSION['message'] = 'reset password success';
					header('Location:'.$link.'/user/reset-password.php');
					exit();
				}
				else
				{
							...
							<div class="col-md-12">
								<div class="auth-form-wrapper px-4 py-5">
									...
									<form method="post" class="forms-sample">
										...
										<a href="<?php echo $link.'/user/login.php';?>" class="d-block mt-3 text-muted text-center">Never mind, I remember my password.</a>
									</form>
								</div>
							</div>
						</div>
